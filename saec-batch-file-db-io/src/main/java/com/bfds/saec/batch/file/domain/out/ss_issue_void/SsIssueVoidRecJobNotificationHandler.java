...
@Component("SsIssueVoidRecJobNotificationHandler")
public class SsIssueVoidRecJobNotificationHandler extends AbstractJobNotificationHandler {
    ...
    protected Map<String, Object> getNotificationModel(JobExecution jobExecution) {
        ...
        ret.put(MAIL_SUBJECT, getNotificationMailSubject());
    ...
    protected String getVelocityTemplate(JobExecution jobExecution) {
        return "vm/SsCashedCheckRecJobNotification.vm";
    }

    private String getQuery() {
        String jpaQl =
                "select c.dda, count(c.id),  sum(c.checkAmount)  " +
                "from com.bfds.saec.batch.file.domain.out.ss_issue_void.SsIssueVoidRec c " +
                " where c.jobExecutionId = :jobExecutionId "+
                " group by c.dda";
        return jpaQl;
    }
	
	private String getNotificationMailSubject() {
		SimpleDateFormat formatter = new SimpleDateFormat("MMddyyyy");
		String currentMonthDateYear = formatter.format(new Date());
		String subject = "SSC issue/void/cancel_" + currentMonthDateYear + "";
		return subject;
	}
}
